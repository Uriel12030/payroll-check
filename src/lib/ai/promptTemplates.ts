...
## שאלות לכלול במייל
${questionsStr || '(אין שאלות ספציפיות — כתוב שאלות כלליות מתאימות לסוג התיק: שכר, שעות נוספות, פיצויים, תנאי סיום עבודה)'}

## מסמכים לבקש
${docsStr || '(אין מסמכים ספציפיים — בקש מסמכים רלוונטיים: תלושי שכר, חוזה עבודה, מכתבי פיטורין)'}
...
- חובה: השדה suggested_text חייב להכיל את גוף האימייל המלא — לעולם אל תשאיר אותו ריק. אם אין שאלות ספציפיות, כתוב אימייל ראשוני כללי עם שאלות רלוונטיות לסוג התיק.
